...
preprocess_gender <- function(file_name, model_name){
  df = read.csv(file_name) %>% 
    ...
    mutate(fem = as.numeric(Feminine)) %>%
    select(-Feminine) %>% 
    mutate(model = model_name)
  return(df)
}

gpt4v = preprocess_gender('../../Data/GPT-4V/Gender/study_2.csv', "GPT-4V")
gpt4o = preprocess_gender('../../Data/GPT-4o/Gender/study_2.csv', "GPT-4o")
gpt4omini = preprocess_gender('../../Data/GPT-4o-mini/Gender/study_2.csv', "GPT-4o-mini")
...
kresult <- searchK(docs, 
                   ...
                   c(3:15), 
                   interactions = FALSE,
                   prevalence = ~gender + fem + gender * fem + model,
                   data = meta, 
                   cores = 4)
...
set.seed(1048596)
summary(estimateEffect(c(1) ~ gender + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(2) ~ gender + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(3) ~ gender + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(4) ~ gender + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(5) ~ gender + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(6) ~ gender + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(7) ~ gender + model, fit, metadata = meta))
...
set.seed(1048596)
summary(estimateEffect(c(2) ~ fem + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(3) ~ fem + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(4) ~ fem + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(5) ~ fem + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(6) ~ fem + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(7) ~ fem + model, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(8) ~ fem + model, fit, metadata = meta))
