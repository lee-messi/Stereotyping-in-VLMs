...
load('../stm_study_2_femininity.RData')

# Plot interaction effect ------------------------------------------------------

plot.int <- function(topic_number, topic_label){
  
  formula_str <- paste("c(", topic_number, ") ~ gender * fem")
...
topic.2.int <- plot.int(2, "Gardening")
topic.3.int <- plot.int(3, "Software Development")
topic.4.int <- plot.int(4, "Art")
topic.5.int <- plot.int(5, "Stars")
topic.6.int <- plot.int(6, "Firefighting")
topic.7.int <- plot.int(7, "Ocean")
topic.8.int <- plot.int(8, "Writing")
...
# Save plot
ggsave("stm_interactions_study_2_femininity.pdf", 
