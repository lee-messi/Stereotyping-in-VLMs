...
load('../Data/Race/GPT-4v/gpt4v.RData')
load('../Data/Race/GPT-4o/gpt4o.RData')
load('../Data/Race/GPT-4o-mini/gpt4omini.RData')

# Define Function --------------------------------------------------------------

new.variables <- function(df, encoder_name){
  df = df %>% 
    mutate(race = case_when(race == "black" ~ "African Americans",
...
ggplot(all_cosines, aes(x = race, y = cosine, color = race)) + 
...
ggsave("homogeneity_race.pdf",width = 9, height = 6, 
