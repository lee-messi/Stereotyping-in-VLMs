...
mpnetbase.gender.proto <- lmer(cosine ~ 1 + gender + proto + (1|pair_id), 
                               data = mpnetbase, 
                               control = lmerControl(optimizer = "nmkbw", 
                                                     calc.derivs = FALSE))

summary(mpnetbase.gender.proto) # Summary output of model including prototypicality
logLik(mpnetbase.gender.proto)

mpnetbase.interaction <- lmer(cosine ~ 1 + gender * proto + (1|pair_id), 
                              data = mpnetbase, 
                              control = lmerControl(optimizer = "nmkbw", 
                                                    calc.derivs = FALSE))
...
distilroberta.gender.proto <- lmer(cosine ~ 1 + gender + proto + (1|pair_id), 
                                   data = distilroberta, 
                                   control = lmerControl(optimizer = "nmkbw", 
                                                         calc.derivs = FALSE))

summary(distilroberta.gender.proto) # Summary output of model including protoulinity
logLik(distilroberta.gender.proto)

distilroberta.interaction <- lmer(cosine ~ 1 + gender * proto + (1|pair_id), 
                                  data = distilroberta, 
                                  control = lmerControl(optimizer = "nmkbw", 
                                                        calc.derivs = FALSE))
...
allminilm.gender.proto <- lmer(cosine ~ 1 + gender + proto + (1|pair_id), 
                               data = allminilm, 
                               control = lmerControl(optimizer = "nmkbw", 
                                                     calc.derivs = FALSE))

summary(allminilm.gender.proto) # Summary output of model including protoulinity
logLik(allminilm.gender.proto)

allminilm.interaction <- lmer(cosine ~ 1 + gender * proto + (1|pair_id), 
                              data = allminilm, 
                              control = lmerControl(optimizer = "nmkbw", 
                                                    calc.derivs = FALSE))
...
save.image('mixed_models_study_2.RData')
