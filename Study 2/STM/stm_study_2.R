...
data = read.csv('../Data/study_2.csv') %>% 
  mutate(image = substr(image, 5, 10)) %>% 
  left_join(cfd %>% select(Model, Masculine, Feminine), by = c("image" = "Model")) %>%
  rename(masc = Masculine, fem = Feminine) %>%
  mutate(masc = as.numeric(masc),
         fem = as.numeric(fem),
         proto = if_else(gender == "male", masc, if_else(gender == "female", fem, NA_real_))) %>%
  select(-c(masc, fem))
...
kresult <- searchK(docs, 
                   ...
                   c(3:15), 
                   interactions = FALSE,
                   prevalence = ~gender + proto + gender * proto,
                   data = meta, 
                   cores = 4)
...
# save.image('stm_study_2.RData')
load('stm_study_2.RData')
...
set.seed(1048596)
summary(estimateEffect(c(1) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(2) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(3) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(4) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(5) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(6) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(7) ~ gender * proto, fit, metadata = meta))

set.seed(1048596)
summary(estimateEffect(c(8) ~ gender * proto, fit, metadata = meta))
